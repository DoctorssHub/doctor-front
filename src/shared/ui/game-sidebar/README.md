...
`GameSidebar` is a shared UI layout shell for game control panels. It owns only
the sidebar container styling and responsive placement. It must not own game
state, betting state, validation, or game-specific controls.
...
Feature folders should provide game-specific containers. For Plinko, that is
`src/features/plinko/ui/sidebar/PlinkoSidebar.tsx`.
