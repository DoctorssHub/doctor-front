...
export function RewardCounter() {
  ...
  return (
    <div className="flex items-center gap-0.5 rounded-lg bg-(--color-page-raised) pr-3 pl-5 py-3 laptop:max-large:w-[450px] tablet:max-laptop:w-[398px] tablet:max-laptop:py-[9px] tablet:max-laptop:pr-[9px] tablet:max-laptop:pl-4 max-tablet:w-[255px] max-tablet:py-[6px] max-tablet:pr-[6px] max-tablet:pl-[10px]">
      <span className="mr-5 text-[36px] font-black text-(--color-brand) laptop:max-large:text-[33px] tablet:max-laptop:text-[29px] max-tablet:mr-2 max-tablet:text-[19px]">
        $
      </span>
      {rewardCharacters.map((character, index) =>
        character === "," ? (
          <span
            className=" grid h-1 w-1 place-items-end text-[20px] font-black leading-none text-(--color-text-primary) tablet:max-laptop:text-[29px] max-tablet:text-[19px]"
            key={`separator-${index}`}
          >
            {character}
          </span>
