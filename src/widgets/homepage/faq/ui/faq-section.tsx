    ...
    <section className="mx-auto flex w-full max-w-[928px] flex-col items-center gap-4">
      ...
      <div className="flex w-full flex-col gap-2">
        {questions.map((question, index) => (
          <details className="group rounded-xl bg-[var(--color-surface)] p-4 text-left" key={`${question}-${index}`} open={index === 1}>
            <summary className="flex cursor-pointer list-none items-center justify-between text-sm font-bold text-white">
              {question}
              <span className="text-[var(--color-text-muted)] group-open:rotate-180">v</span>
            </summary>
            <p className="mt-3 pr-5 text-sm leading-5 text-[var(--color-text-muted)]">
